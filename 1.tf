...
#creating security group
 resource "aws_security_group" "my-ssh-http" {
   name    = "my-ssh-http"
...
resource "aws_instance"  "myin1" {
   ...
   security_groups = ["${aws_security_group.my-ssh-http.name}"]
...
#creating s3 bucket
resource "aws_s3_bucket" "kanika12312-bucket" {
   depends_on = [
    aws_volume_attachment.ebs_att,
  ]
    bucket  = "kanika12312-bucket"
    acl     = "public-read"
...
resource "aws_s3_bucket_object" "objectk-upload" {
    bucket  = aws_s3_bucket.kanika12312-bucket.bucket
    key     = "myimage.jpeg"
...
locals {
    s3_origin_id = "${var.var1}${aws_s3_bucket.kanika12312-bucket.bucket}"
...
origin {
        domain_name = aws_s3_bucket.kanika12312-bucket.bucket_domain_name
...
resource "null_resource" "launchsite"  {

depends_on =[ aws_cloudfront_distribution.s3_distribution,aws_instance.myin1,aws_volume_attachment.ebs_att, ] 
  ...
  provisioner "local-exec" {
   command = "start  brave  ${aws_instance.myin1.public_ip}"
   }

  provisioner "remote-exec"{ 
  inline=[ "sudo su <<EOF",
         "echo \"<img src= 'http://  ${aws_cloudfront_distribution.s3_distribution.domain_name}/aws.jpg' height='300'>\"   >> /var/www/html/index.php",
		 "EOF",
  ] 
 } 
}
